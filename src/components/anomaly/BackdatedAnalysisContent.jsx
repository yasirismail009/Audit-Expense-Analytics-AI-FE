...
export default function BackdatedAnalysisContent({ data, distributionData, anomalySummary }) {
  return (
    <Box>
      ...
      {distributionData && (
        <Box sx={{ mb: 3, p: 2, backgroundColor: '#fff3e0', borderRadius: 2, border: '1px solid #ffcc02' }}>
          <Typography variant="h6" sx={{ mb: 2, fontWeight: 'bold', color: '#e65100' }}>
            📊 Backdated Entries Distribution Overview
          </Typography>
      ...
      <Alert severity="info" sx={{ mb: 2 }} icon={<InfoIcon />}>
        ...
        <Typography variant="body2" sx={{ mt: 1 }}>
          Analysis of transactions posted on dates different from document dates.
        </Typography>
      </Alert>
          ...
          <Grid item xs={12} md={3}>
            <Paper sx={{ p: 2, textAlign: 'center', bgcolor: '#fff3e0' }}>
              <Typography variant="h6" color="#e65100">
                {data.summary.total_backdated || 0}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                Backdated Entries
              </Typography>
          ...
          <Grid item xs={12} md={3}>
            <Paper sx={{ p: 2, textAlign: 'center', bgcolor: '#fce4ec' }}>
              <Typography variant="h6" color="#c2185b">
                {data.summary.avg_days_difference || 0}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                Avg Days Difference
              </Typography>
      ...
      {data.backdated_entries && data.backdated_entries.length > 0 && (
        <TableContainer component={Paper} sx={{ mb: 3 }}>
          <Table>
            ...
            <TableBody>
              {data.backdated_entries.map((entry, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    <Chip 
                      ...
                      size="small"
                      color={Math.abs(entry.days_difference) > 30 ? 'error' : Math.abs(entry.days_difference) > 7 ? 'warning' : 'success'}
                    />
                  </TableCell>
