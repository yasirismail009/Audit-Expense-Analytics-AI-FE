...
export default function HolidayAccountChart({ data, currency = 'SAR' }) {
  ...
  const chartData = data?.visualizations?.chart_data?.holiday_by_account;

  if (!data || !chartData || !chartData.labels || !chartData.data) {
    return (
      <Card sx={{ height: '100%', borderRadius: 3, boxShadow: 2 }}>
        <CardContent>
          <Typography variant="subtitle2" sx={{ mb: 2 }}>Holiday Activity by Account</Typography>
          <Box sx={{ height: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Typography variant="body2" color="text.secondary">No holiday account data available</Typography>
          </Box>
        </CardContent>
      </Card>
    );
  ...
  const transformedData = chartData.labels.map((account, index) => ({
    account: account,
  ...
  if (holidayEntries.length > 0) {
    transformedData.forEach(item => {
      const accountHolidays = holidayEntries.filter(entry => entry.account === item.account);
      item.totalAmount = accountHolidays.reduce((sum, entry) => sum + (entry.amount || 0), 0);
      item.uniqueHolidays = new Set(accountHolidays.map(entry => entry.holiday_name)).size;
    });
  }
